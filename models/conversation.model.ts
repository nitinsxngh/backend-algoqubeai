...
ConversationSchema.methods.addMessage = function(role: string, content: string, tokensUsed: number = 0) {
  this.messages.push({
    role,
    content,
    timestamp: new Date(),
...
ConversationSchema.methods.updateStatus = function(status: string) {
  this.status = status;
...
ConversationSchema.statics.getByChatbox = function(chatboxName: string, options: any = {}) {
...
ConversationSchema.statics.getStats = function(chatboxName: string, options: any = {}) {
  const matchQuery: any = { chatboxName };
  
  if (options.startDate || options.endDate) {
    matchQuery.timestamp = {};
    if (options.startDate) matchQuery.timestamp.$gte = new Date(options.startDate);
    if (options.endDate) matchQuery.timestamp.$lte = new Date(options.endDate);
  }

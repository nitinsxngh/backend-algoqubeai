...
router.get('/', async (req, res) => {
    try {
        ...
        const chatboxQuery = { createdBy: userId };
        if (chatboxName) {
            chatboxQuery.name = { $regex: chatboxName, $options: 'i' };
        }
        const chatboxes = await chatbox_model_1.default.find(chatboxQuery).select('name organizationName');
        const chatboxNames = chatboxes.map(cb => cb.name);
        // Get all conversations for user's chatboxes
        const query = { chatboxName: { $in: chatboxNames } };
        // Apply filters
        ...
        const allConversations = await conversation_model_1.default.find(query)
            .sort({ timestamp: -1 });
 ...
 */
router.get('/:chatboxName', async (req, res) => {
    ...
    try {
        ...
        const totalConversations = await conversation_model_1.default.countDocuments(query);
        const pageNum = parseInt(page);
        const limitNum = parseInt(limit);
        return res.status(200).json({
 ...
 */
router.get('/:chatboxName/:conversationId', async (req, res) => {
 ...
 */
router.put('/:chatboxName/:conversationId/status', async (req, res) => {
    const { chatboxName, conversationId } = req.params;
 ...
 */
router.delete('/:chatboxName/:conversationId', async (req, res) => {
    const { chatboxName, conversationId } = req.params;
    try {
        const userId = req.user?.id;
        const chatbox = await chatbox_model_1.default.findOne({
 ...
 */
router.post('/:chatboxName/export', async (req, res) => {
    const { chatboxName } = req.params;
 ...
 */
router.get('/:chatboxName/stats', async (req, res) => {
    const { chatboxName } = req.params;

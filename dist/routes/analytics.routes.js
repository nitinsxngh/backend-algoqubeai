...
function ensureAnalytics(chatbox) {
 ...
 */
router.post('/initiate/:name', async (req, res) => {
    const { name } = req.params;
    try {
        const chatbox = await chatbox_model_1.default.findOne({ name });
        if (!chatbox)
 ...
 */
router.post('/message/:name', async (req, res) => {
    const { name } = req.params;
    ...
    try {
        ...
        const user = await user_model_1.default.findById(chatbox.createdBy);
        if (user && user.tokens) {
 ...
 */
router.post('/complete/:name', async (req, res) => {
    const { name } = req.params;
 ...
 */
router.post('/session/:name', async (req, res) => {
    const { name } = req.params;
    // Handle both JSON and string data from sendBeacon
    let duration;
    if (typeof req.body === 'string') {
 ...
 */
router.get('/conversation/:name/:conversationId', async (req, res) => {
 ...
 */
router.put('/conversation/:name/:conversationId/status', async (req, res) => {
    const { name, conversationId } = req.params;
 ...
 */
router.delete('/conversation/:name/:conversationId', async (req, res) => {
    const { name, conversationId } = req.params;
    try {
        const chatbox = await chatbox_model_1.default.findOne({ name });
        if (!chatbox)
        ...
        if (!conversation)
            return res.status(404).json({ message: 'Conversation not found' });
        await conversation_model_1.default.findByIdAndDelete(conversation._id);
        return res.status(200).json({
 ...
 */
router.post('/conversations/:name/export', async (req, res) => {
    const { name } = req.params;
    ...
    try {
        const chatbox = await chatbox_model_1.default.findOne({ name }).select('organizationName');
        if (!chatbox)
            return res.status(404).json({ message: 'Chatbox not found' });
        const query = { chatboxName: name };
        // Apply filters
        ...
        const conversations = await conversation_model_1.default.find(query).sort({ timestamp: -1 });
        if (format === 'json') {
            return res.status(200).json({
                chatboxName: name,
 ...
 */
router.get('/conversations/:name/stats', async (req, res) => {
    const { name } = req.params;
    const { startDate, endDate } = req.query;
    try {
        const chatbox = await chatbox_model_1.default.findOne({ name }).select('analytics');
        if (!chatbox)
            return res.status(404).json({ message: 'Chatbox not found' });
        const options = { startDate, endDate };
        const matchQuery = { chatboxName: name };
        if (options.startDate || options.endDate) {
        ...
        const stats = await conversation_model_1.default.aggregate([
            { $match: matchQuery },
            {
                $group: {
                    _id: null,
        ...
        const conversations = await conversation_model_1.default.find(query).select('timestamp');
